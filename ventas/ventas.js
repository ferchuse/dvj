...
$(document).ready( function onLoad(){
	...
	$(".tipo_precio").change( aplicarMayoreo);
	...
	//Autocomplete Clientes https://github.com/devbridge/jQuery-Autocomplete
	$("#nombre").autocomplete({
		serviceUrl: "../clientes/clientes_autocomplete.php",   
		onSelect: function(eleccion){
			console.log("Elegiste: ",eleccion);
...
function agregarProducto(producto){
	...
	var tipo_precio = $(".tipo_precio:checked").val();
	...
	if($existe.length > 0){
		...
		<input hidden class="precio_medico" value='${producto['precio_medico']}'>
		...
		<td class="col-sm-1"><input readonly type="number" class='precio form-control' value='${producto[tipo_precio]}'> </td>
...
function guardarVenta(event){
	...
	if($("#nombre").val() == ""){
		alertify.error("Elige un Cliente");
...
function imprimirTicket(id_ventas){
	...
	var win = window.open("imprimir_nota_venta.php?id_ventas=" + id_ventas , '_blank');
  win.focus();
	
	// $.ajax({
	// url: "impresion/imprimir_nota_venta.php",
	// data:{
	// id_ventas : id_ventas
	// }
	// }).done(function (respuesta){
...
function aplicarMayoreo(){
	var tipo_precio = $(".tipo_precio:checked").val();
	console.log("cambio de precio",tipo_precio);
	
	$("#tabla_venta tbody tr").each(function(index, item){
		
		$(item).find(".precio").val($(item).find("."+tipo_precio).val());
	});
